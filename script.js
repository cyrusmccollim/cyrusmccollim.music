const audioData = {
      "commissioned": {
            ...
            "Orchestral": [
                  "commissioned/orchestral/Villian.mp3"
            ]
      },
      "original": {
            "Acoustic": [
...
function generateStaticPeaks(path, barCount) {
  var seed = 5381;
  ...
  for (var i = 0; i < barCount; i++) {
    ...
    var base = (seed % 1000) / 1000;
    var pos = i / barCount;
    var envelope = 0.4 + 0.6 * Math.sin(pos * Math.PI);
    var val = 0.12 + base * 0.65 * envelope;
    peaks.push(val);
  }
  return peaks;
}
...
function loadWaveform(path, canvas) {
  if (waveformCache[path]) {
    ...
    if (durations[path]) {
      ...
      if (el) {
        ...
        if (td) td.textContent = "0:00 / " + formatTime(durations[path]);
      }
    }
    return;
  }
  ...
  tempAudio.src = AUDIO_BASE + path;
  tempAudio.addEventListener("loadedmetadata", function() {
    durations[path] = tempAudio.duration;
    var el = canvas.closest(".track-item");
    if (el) {
      var td = el.querySelector(".time-display");
      if (td) td.textContent = "0:00 / " + formatTime(tempAudio.duration);
    }
    tempAudio.src = "";
  });
}
...
function renderTracks(tab) {
  ...
  if (!data || Object.keys(data).length === 0) {
    ...
    empty.textContent = "Uploading soon!";
